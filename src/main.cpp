        ...
        for (unsigned int y = min_y; y <= max_y; ++y) {
            for (unsigned int x = min_x; x <= max_x; ++x) {
                ...
                if (value!=0) continue;
                    ...
                    Bbox_3 box(center.x()-voxel_grid.resolution/2, center.y()-voxel_grid.resolution/2, center.z()-voxel_grid.resolution/2,
                               center.x()+voxel_grid.resolution/2, center.y()+voxel_grid.resolution/2, center.z()+voxel_grid.resolution/2);
                    CGAL::do_intersect(box, triangle);
...
int main() {
    ...
    const std::string output_file = "../data/objs/ifc2_voxel.obj";
...
//    std::cout << "building_num: "<< voxel_grid.buildings.size() <<std::endl;
    ...
    voxel_grid.voxel_to_obj(buildings, origin, output_file);
